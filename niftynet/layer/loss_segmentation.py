...
class LossFunction(Layer):
    ...
    def __init__(self, n_class,loss_type='Dice',loss_func_params=None, name='loss_function'):
    ...
    def dicee(self, prediction, ground_truth, weight_map=None):
        """
        ...
        #self.PREDICTION = tf.Variable(p, trainable=False)
        #self.GROUNDTRUTH = tf.Variable(gt,trainable=False)
        
        #self.PREDICTION = tf.get_variable("pred",[865280, 14])
        
        print("after variable initializations pre y g")

        prediction = tf.nn.softmax(prediction)
        
        self.CONT = tf.to_int64(tf.shape(ground_truth)[0])
        ...
        print("size of prediction: ", prediction.get_shape()) #(512,14)   #(865280, 14)                        
        print("size of ground_truth: ", one_hot.get_shape()) # (512,) (865280,)

        self.PREDICTION = prediction
        self.GROUNDTRUTH = one_hot
        #self.SUMA =  tf.sparse_reduce_sum(one_hot * prediction, reduction_axes=[0])
        self.SUMA = one_hot * prediction

        #self.SUBS = tf.slice(one_hot, [0,0], [1,1])
        #print("size of slicing one_hot: ", self.SUBS.get_shape()    )
        ...
        if weight_map is not None:
            print("WWWWWWWWWWWWWWWWe have weight map")
            ...
            #NO WEIGHTMAP
            #SUBSTRACTING THE TENSORS
            print("WWWWWWWWWWWWWWWWe DONTT have weight map")
            dice_numerator = 2.0 * tf.sparse_reduce_sum(
    ...
    def cross_entropy(self, prediction, ground_truth, weight_map=None):
        ...
        """
        print("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEntering cross_entropy")
        ground_truth = tf.to_int64(ground_truth)
        prediction = tf.cast(prediction, tf.float32)
        entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prediction, labels=ground_truth)
        if weight_map is not None:
        ...
        Entropy = tf.convert_to_tensor(entropy, name="MYTENSORENTROPY")        
        #print(Entropy.name)
        reduced = tf.reduce_mean(entropy)
        Reduced = tf.convert_to_tensor(reduced, name="MYTENSORENTROPYREDUCED")
        print("RRRRRRRRRRRRRRRRRRRRRRRRreduced: ", Reduced.name)
        return reduced
    ...
    def layer_op(self,prediction,ground_truth=None,weight_map=None, var_scope=None, ):
        """
        ...
        with tf.device('/cpu:0'):
            ...
            print("Entering segmentation layer op")            
            ...
            for pred in prediction:
                ...
                if self._loss_func_params:
                    ...
                    #data_loss.append(self.dicee(pred, ground_truth, weight_map,**self._loss_func_params))
                    data_loss.append(self.cross_entropy(pred, ground_truth, weight_map,**self._loss_func_params))
                else:
                    data_loss.append(self.dicee(pred, ground_truth, weight_map))
                    #data_loss.append(self._data_loss_func(pred, ground_truth, weight_map))
                    data_loss.append(self.cross_entropy(pred, ground_truth, weight_map))
            return tf.reduce_mean(data_loss)

    def return_loss_args(self):        
...
def generalised_dice_loss(prediction,ground_truth,weight_map=None,type_weight='Square'):    
    ...
    :return: the loss
    """
def sensitivity_specificity_loss(prediction,ground_truth,weight_map=None,r=0.05):
    ground_truth = tf.to_int64(ground_truth)
    n_voxels = ground_truth.get_shape()[0].value
    n_classes = prediction.get_shape()[1].value
    prediction = tf.nn.softmax(prediction)
...
def wasserstein_disagreement_map(prediction, ground_truth, M):
    ...
    return wass_dis_map
def generalised_wasserstein_dice_loss(prediction,ground_truth,weight_map=None):
    """
